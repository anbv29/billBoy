...
function makeRequest(previous, current) {
  ...
  return {
    ...
    generationConfig: { responseFormat: { text: {
      ...
      schema: { type: 'object', properties: { previous: billSchema, current: billSchema },
...
async function extractBills(previous, current, apiKey, fetchImpl = fetch) {
  ...
  if (!response.ok) throw new Error('Gemini HTTP ' + response.status + ': ' + body.slice(0, 300));
...
async function run(previousName, currentName, apiKey, fetchImpl = fetch, reportPrefix = 'report') {
  ...
  if (fs.existsSync(jsonPath)) {
    try {
      ...
      if (saved.sources?.previous?.hash === previous.hash &&
